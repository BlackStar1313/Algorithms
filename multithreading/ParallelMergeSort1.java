...
public final class ParallelMergeSort1<T extends Comparable<? super T>> implements Runnable {
	 ...
	 * @param threads
	 */
	private ParallelMergeSort1(T[] array, int startIndex, int endIndex, int threads) {
		originalArray = array;
		from = startIndex;
		to = endIndex;
		nbThreads = threads;
	...
	public static <T extends Comparable<? super T>> ParallelMergeSort1<T> newInstance(T[] arr, int startIndex, int endIndex, int threads) {
		ParallelMergeSort1<T> sort = new ParallelMergeSort1<>(arr, startIndex, endIndex, threads);
		return sort;
	}
	 ...
	 */
	public void parallelTopDownSplit(T[] a , int startIndex, int endIndex, int availableThreads) {
		...
		if(endIndex <= startIndex) {
			...
			if(availableThreads <= 1) {
				...
				Thread thread1 = new Thread(newInstance(a, startIndex, midpoint, availableThreads/2));
				thread1.start();

				Thread thread2 = new Thread(newInstance(a, midpoint + 1, endIndex, availableThreads/2));
				thread2.start();
