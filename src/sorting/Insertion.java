...
public class Insertion {
	...
	public static <T extends Comparable<? super T>> void optimizedSort(T[] array, int startIndex, int endIndex) {
		for (int i = startIndex + 1; i <= endIndex; i++) {
			T key = array[i];
			int j = i;
			while((j > startIndex) && GenericHelper.isLess(key, array[j - 1])) {
				array[j] = array[j - 1];
				j--;
			}
			array[j] = key;
